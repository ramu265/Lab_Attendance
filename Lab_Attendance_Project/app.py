...
def init_db():
    ...
    # UPDATED SUBMISSIONS TABLE: Added status and error_message to track all attempts
    ...
    # IMPORTANT: If table exists but columns are missing, add them (Migration)
    try:
        c.execute("ALTER TABLE submissions ADD COLUMN status TEXT")
        c.execute("ALTER TABLE submissions ADD COLUMN error_message TEXT")
    except:
...
def view_results(student_id):
    ...
    # Get all distinct tasks the student attempted
    c.execute("""SELECT DISTINCT tasks.id, tasks.question 
                 FROM submissions JOIN tasks ON submissions.task_id = tasks.id 
                 WHERE submissions.student_id=?""", (student_id,))
    tasks_attempted = c.fetchall()

    final_data = []
    for task in tasks_attempted:
        task_id, question = task
        # Get all attempts for this specific task
        c.execute("""SELECT submitted_code, time_spent_seconds, submission_date, status, error_message 
                     FROM submissions WHERE student_id=? AND task_id=? ORDER BY id DESC""", (student_id, task_id))
        attempts = c.fetchall()
        final_data.append({"question": question, "attempts": attempts})

    conn.close()
    return render_template("student_submissions.html", data=final_data, student_name=student_name)
...
def compiler(task_id):
    ...
    if request.method == "POST":
        ...
        with open(filename, "w") as f: f.write(submitted_code)

        try:
            # Run the code
            output = subprocess.check_output(
                [sys.executable, filename],
                input=task[2], text=True, timeout=5, stderr=subprocess.STDOUT
            ).strip()

            if output == task[3].strip():
                status = "Success"; error_msg = ""
                result = f"✅ Correct! Time Spent: {time_spent}s. Attendance Marked."
                # Attendance record
                c.execute("SELECT * FROM attendance WHERE student_id=? AND date=?", (session["student_id"], today))
                if not c.fetchone():
                    c.execute("INSERT INTO attendance(student_id,date) VALUES(?,?)", (session["student_id"], today))
            else:
                status = "Error"; error_msg = f"Wrong Output. Got: {output}"
                result = "❌ Wrong Output!"

        except subprocess.CalledProcessError as e:
            status = "Error"; error_msg = e.output
            result = f"⚠️ Runtime Error: {error_msg}"
        except Exception as e:
        ...
        # SAVE EVERY ATTEMPT
        c.execute("""INSERT INTO submissions(student_id, task_id, submitted_code, 
                     time_spent_seconds, submission_date, status, error_message) 
                     VALUES(?,?,?,?,?,?,?)""",
                  (session["student_id"], task_id, submitted_code, time_spent, today, status, error_msg))
        conn.commit()
...
def download_pdf():
    ...
    elements=[]
    elements.append(Paragraph(f"Class Report ({start} to {end})", getSampleStyleSheet()["Heading2"]))
